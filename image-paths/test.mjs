...

{
  ...
  try {
    ...
    await writeFile(join(root, 'diagram.svg'), '<svg xmlns="http://www.w3.org/2000/svg"><rect/></svg>')
    await writeFile(join(root, 'fake.svg'), 'not an svg at all')
    const svg = await host.resolveImageTarget('diagram.svg', root)
    assert.equal(svg.status, 200, 'an svg is an image too: architecture diagrams export as vector')
    assert.equal(svg.mediaType, 'image/svg+xml')
    assert.equal((await host.resolveImageTarget('fake.svg', root)).status, 415, 'a file merely named .svg is refused')
...

{
  ...
  // A sandboxed canvas page has an opaque origin; its own requests are trusted
  // by the page they came from, which is how an artifact shows a local image.
  assert.equal(host.sameOrigin({ headers: { host: '127.0.0.1:3080', 'sec-fetch-site': 'cross-site', referer: 'http://127.0.0.1:3080/plugin/markdown-preview/canvas?path=a.html' } }), true)
  assert.equal(host.sameOrigin({ headers: { host: '127.0.0.1:3080', 'sec-fetch-site': 'cross-site', referer: 'http://evil.test/plugin/x' } }), false, 'a foreign referrer grants nothing')
